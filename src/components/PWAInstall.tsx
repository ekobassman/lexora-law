...
/** Restituisce la traduzione o il fallback EN, mai la chiave raw */
function pwaLabel(key: string, enFallback: string): string {
  const v = i18n.t(key);
  if (!v || v === key || String(v).startsWith('pwa.')) return enFallback;
  return String(v);
}
...
export function PWAInstall() {
  ...
  const handleIOSAddToHome = () => {
    handleIOSDismiss();
    toast(pwaLabel('pwa.iOSGuide.hint', 'Tap Share → Add to Home'));
  };

  // iOS: modal minimale (solo logo, titolo, bottone, X)
  return (
    <Dialog open={showIOSModal} onOpenChange={handleIOSOpenChange}>
      ...
      >
        ...
        <div className="p-6 space-y-6 pb-[calc(1.5rem+env(safe-area-inset-bottom,0px))]">
          <div className="flex flex-col items-center text-center space-y-3">
            ...
            <h2 className="text-2xl font-bold text-gray-900">
              {pwaLabel('pwa.iOSGuide.installTitle', 'Install Lexora')}
            </h2>
          </div>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 rounded-xl bg-gray-100 text-gray-900 p-4 text-base font-medium hover:bg-gray-200 active:bg-gray-300 transition-colors"
          >
            <Plus className="h-5 w-5 shrink-0" />
            {pwaLabel('pwa.iOSGuide.addToHomeButton', 'Add to Home')}
          </button>
        </div>
      </DialogContent>
    </Dialog>
